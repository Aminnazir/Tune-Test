...
function add_button($actions){

    $result = '';

    foreach ($actions as $action) {
        if($action->type == 'add'){
            $result .='  <a data-title="'.$action->title.'" data-ajax="'.($action->ajax ? $action->ajax : 0).'" href="'.route($action->route).'" class="dt-button add-new btn btn-primary dt-button buttons-collection mx-3"><span>
                            <i class="bx bx-plus me-0 me-sm-2"></i>
                            <span class="d-none d-lg-inline-block">'.$action->label.'</span>
                        </span>
                            </a>';
        }
    }
...
if (!function_exists('formConditionalFieldMap')) {

    function    formConditionalFieldMap($options = [])
    {
        $response = array();
        if(is_array($options)) {
            $response = [];
            foreach ($options as $key => $option) {
                $response[]['data-cond'] = '[name=' . $option['field'] . '] ' . $option['op'] . ' ' . $option['value'] . '';
            }
        }
